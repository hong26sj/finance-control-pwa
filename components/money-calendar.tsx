...
export function MoneyCalendar() {
  ...
  const filteredMonthRows = useMemo(() => currentMonthRows.filter((row) => {
    ...
    if (cardFilter !== '전체' && row.card !== cardFilter) return false
    return true
  }), [currentMonthRows, categoryFilter, cardFilter])
  const cards = useMemo(() => [...new Set(rows.map((row) => row.card).filter(Boolean))].sort(), [rows])
  const byDate = useMemo(() => {
    const map = new Map<string, Transaction[]>()
    filteredMonthRows.forEach((row) => map.set(row.date, [...(map.get(row.date) || []), row]))
    return map
  }, [filteredMonthRows])
  ...
  const monthTotal = filteredMonthRows.reduce((sum, row) => sum + row.amount, 0)
    ...
    <section className="money-calendar-page" aria-label="머니 캘린더">
      ...
      <article className="panel calendar-panel">
        ...
        <div className="calendar-filters">
          <label>카테고리<select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}><option>전체</option>{CATEGORIES.map((category) => <option key={category}>{category}</option>)}</select></label>
          <label>결제수단<select value={cardFilter} onChange={(e) => setCardFilter(e.target.value)}><option>전체</option>{cards.map((card) => <option key={card}>{card}</option>)}</select></label>
        </div>
