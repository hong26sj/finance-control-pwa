...
export function MoneyCalendar() {
  ...
  return <>
    ...
    <section className="money-calendar-page" aria-label="머니 캘린더">
      ...
      <article className="panel calendar-day-panel"><div className="calendar-day-head"><div><span className="eyebrow">DAILY DETAIL</span><h3>{Number(selected.slice(5, 7))}월 {Number(selected.slice(8, 10))}일</h3></div><div><b>{won(selectedTotal)}</b><small>{selectedRows.length}건</small></div></div>{selectedRows.length === 0 ? <div className="calendar-empty-day">선택한 필터에 해당하는 거래가 없습니다.</div> : <div className="calendar-day-list">{selectedRows.map((row) => <div key={row.id} data-transaction-id={row.id}><span className="calendar-category-dot" style={{ background: CATEGORY_COLORS[row.category] || '#89948e' }} /><span><b>{row.merchant || row.category}</b><small>{row.category} · {row.card}{row.time ? ` · ${row.time}` : ''}{row.performanceIncluded ? '' : ' · 실적 제외'}</small></span><strong>{won(row.amount)}</strong></div>)}</div>}</article>
    </section>
  </>
}
