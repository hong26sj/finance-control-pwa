function importShortcutTransactionDirectV2_(body) {
  ...
  try {
    ...
    var suggestedCategory = rule && rule.category ? rule.category : shortcutFallbackCategory_(merchant);

    if (shortcutExistsInServer_(snapshot, details, date, time, card, amount, merchantHash)) {
      return { ok: true, status: 'duplicate', merchant: merchant, amount: amount, suggestedCategory: suggestedCategory };
    }
